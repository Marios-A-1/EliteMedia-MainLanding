...
export default async function ClaimOnlinePage({ searchParams }: PageProps) {
  const resolvedSearchParams = await searchParams;
  const sessionId = resolvedSearchParams?.session_id;

  return (
    <ClaimTicketShell
      subtitle="Εισιτήριο Online. Συμπλήρωσε τα στοιχεία σου για να ολοκληρώσεις την καταχώρηση."
    >
      {sessionId ? (
        <ClaimTicketForm tier="online" sessionId={sessionId} />
      ) : (
        <MissingSession />
      )}
    </ClaimTicketShell>

import { ExternalLinkIcon } from "lucide-react";

export default function ForbesQuote() {
  return (
    <section className="mx-auto mt-14 max-w-3xl max-h-300 px-6">
      <a
        href="https://www.forbesgreece.gr/story/3934912/elitemedia-to-elliniko-agency-pou-katalabainei-kalutera-ton-ellina-ap-oso-katalabainei-o-idios-ton-eauto-tou"
        target="_blank"
        rel="noopener noreferrer"
        className="group block rounded-2xl py-8  px-10 border border-gold-500/40 bg-gradient-to-br from-gold-500/30 via-gold-200/70 to-gold-500/50 p-6 transition hover:shadow-lg"
      >
        <blockquote className="text-lg font-serif font-medium text-neutral-800 leading-relaxed">
          “EliteMedia: Το ελληνικό agency που καταλαβαίνει καλύτερα τον Έλληνα απ’ όσο καταλαβαίνει ο ίδιος τον εαυτό του”
        </blockquote>
        
        <div className="mt-6 flex items-center gap-2 opacity-80 justify-end group-hover:opacity-100">
            <span className="text-xs text-neutral-500">Διαβάστε το πλήρες άρθρο στο </span>
          <img
          style={{marginTop: '-6px'}}
            src="/images/Forbes_logo.svg"
            alt="Forbes"
            className="h-4 w-auto"
          />
            {/* <ExternalLinkIcon className="justify-self-end h-4 w-4 -mt-2 -mr-6 text-neutral-500" /> */}
        </div>
      </a>
    </section>
  )
}
